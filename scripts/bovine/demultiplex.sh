...
do
    ...
    grep -e "^$i" input/bovine/Samples.txt | awk '{ print $2"."$3"\t"$5}' > input/bovine/$i.sample.index
...
fastq-multx input/bovine/$i.sample.index $outputdir/cut2/$i.R1.fastq.gz $outputdir/cut2/$i.R2.fastq.gz -o $outputdir/demultiplexed/%.R1.fastq.gz $outputdir/demultiplexed/%.R2.fastq.gz
EOF
    workdir=`pwd`
